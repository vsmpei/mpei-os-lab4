...
awk -v max_value="${max_value}" '{$3=$2; gsub(/0/, "", $3); len=length($3); gsub(/1*/, len, $3)} $3==max_value {print "Никнейм в БАРС: " $1 " Количество посещенных занятий: " $3}' $file_path 
...
#--------------------------------------------------------------------
# проверка соержимого файла для пункта три 
#--------------------------------------------------------------------
